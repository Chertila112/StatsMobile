...
const MatchParticipant = ({ player }: MatchParticipantProps) => {
  return (
    <Animated.View entering={player.teamId === 100 ? FadeInLeft.duration(300).delay(100) : FadeInRight.duration(300).delay(100)} className={`flex-row border rounded-xl w-full gap-1 p-2 bg-dark-3 ${player.teamId === 200 ? 'justify-end' : 'justify-start'}`} >
      {
        player.teamId === 100 && <View className='w-[60px] h-[60px] border-dark-6 border overflow-hidden my-auto'
          style={{
      ...
      <View className={`${player.teamId === 200 ? 'items-end' : 'items-start'} flex-1`}>
        ...
        <Text className='text-dark-6 text-xs'>{`${player.kills}/${player.deaths}/${player.assists}`}</Text>
      ...
      {
        player.teamId === 200 && <View className='w-[60px] my-auto border-dark-6 border h-[60px] overflow-hidden'
          style={{
